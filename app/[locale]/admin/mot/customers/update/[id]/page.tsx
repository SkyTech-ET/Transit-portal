...
export default function CreateCustomerUserPage() {
  ...
  const handleSubmit = async (values: any) => {
    ...
    // Core fields for creating the user
...
Array.from(formData.entries()).forEach(([key, value]) => {
  console.log(key, value);
    ...
    try {
      ...
      message.success("Customer user created successfully!");
      router.push("/admin/mot/customers/services");
    } catch (err) {
      message.error("Failed to create customer user. Ensure all required fields are uploaded.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="p-8">
      <h2 className="text-xl font-semibold mb-6">Create User for Customer</h2>
